...
## 2026-05-13 — AI-Ready Codebase Rubric 문서 작성

### 작업 목적

사용자 요청에 따라 코드베이스가 AI 협업에 얼마나 준비되어 있는지 평가할 수 있는 100점 기준표를 문서화한다.
...
- `docs/ai-ready-codebase-rubric.md`
- `docs/agent-logs/pm.md`

### 한 일

- AI-ready codebase 평가 기준을 7개 카테고리, 총 100점으로 정리했다.
- 각 카테고리에 점수 구간, 평가 기준, 체크 포인트, 감점 예시 또는 권장 산출물을 추가했다.
- evidence 기반 JSON 결과 형태와 ROI 액션 우선순위 기준을 포함했다.

### 왜 그렇게 했는지

- 단순히 "문서가 많다"가 아니라 AI가 실제로 작업을 찾고, 영향 범위를 추적하고, 검증까지 마칠 수 있는지를 평가하기 위해서다.
- 향후 repo audit 자동화나 대시보드 생성 스킬을 만들 때 scoring rubric 파일로 바로 전환할 수 있게 구조화했다.

### 남은 문제

- 아직 실제 Forkcast DeFi repo에 대한 점수 산정은 하지 않았다.
- 자동 채점 스크립트나 HTML 대시보드는 별도 작업으로 분리할 수 있다.
